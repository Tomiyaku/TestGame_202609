...
namespace CodeIcf.AssetManagement.AdressableManagement
{
    ...
    public class AddressableStorage
    {
        ...
        private AsyncOperationHandle AsyncOperationHandle { get; set; }
        ...
        /// <param name="_assetAddress">Assetのアドレス</param>
        public AddressableStorage( AddressableCategory _category, uint _index, string _assetAddress )
        {
            uint identifier = AddressableIdentifier.CreateIdentifier( _category, _index );

            Init( identifier, _assetAddress );
        }
        ...
        /// <param name="_assetAddress">Assetのアドレス</param>
        public AddressableStorage( uint _identifier, string _assetAddress )
        {
            Init( _identifier, _assetAddress );
        }
        ...
        /// <param name="_identifier">Asset識別子</param>
        /// <param name="_assetAddress">Assetのアドレス</param>
        private void Init( uint _identifier, string _assetAddress )
        {
            m_Identifier = _identifier;
            AsyncOperationHandle = Addressables.LoadAssetAsync<Object>( _assetAddress );
        ...
        public async void LoadAssetAsync()
        {
            ...
            {
                m_Asset = AsyncOperationHandle.Result as Object;
                m_State = AssetState.Enable;

                AddressableIdentifier.DivideIdentifier( Identifier, out AddressableCategory category, out _ );
                Debug.Log( "ltdl Load AddressableAsset Success!  Caegory=" + category + " AssetName=" + m_Asset.name + " LoadTime=" + ( Time.unscaledTime - loadStartTime ) );
            }
            else
            {
                m_Asset = null;
                ...
                Debug.Log( "Failed Load AddressableAsset!  Asset Identifier = " + Identifier );
            }
        }
        ...
        /// <summary>
        /// Instantiate()を実行する前のGameObjectを取得
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public GameObject GetBeforeInstantateGameObject() 
        {
            return m_Asset as GameObject;
        }

        /// <summary>
        /// Instantiate()を実行する前のComponentを取得
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public T GetBeforeInstantatePrefab<T>() where T : Object
        {
            GameObject gameObject = GetBeforeInstantateGameObject();
        ...
        public void ReleaseHandle( bool _isForced = false )
        {
            ...
            if( _isForced )
            {
                List<IAssetBundleResource> resourcesList = AsyncOperationHandle.Result as List<IAssetBundleResource>;
