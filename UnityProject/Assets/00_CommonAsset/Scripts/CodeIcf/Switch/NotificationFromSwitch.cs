...
namespace CodeIcf.Switch
{
    ...
    public class NotificationFromSwitch
    {
        ...
        static void NotificationMessageReceived( Notification.Message _message )
        {
            ...
            else if (_message == Notification.Message.ExitRequest)
            {
                Debug.Log("ExitRequest Now: " + _message);
                if (PhotonNetwork.RunnerSpawner.Instance != null && PhotonNetwork.RunnerSpawner.Instance.Runner != null)
                {
                    // オンラインプレイ中だったら接続を切る
                    PhotonNetwork.RunnerSpawner.Instance.ShutdownGame();
                }
