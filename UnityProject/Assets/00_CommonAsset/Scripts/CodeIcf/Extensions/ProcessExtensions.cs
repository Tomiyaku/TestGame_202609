...
namespace CodeIcf.Extensions
{
    ...
    public static class ProcessExtensions
    {
        ...
        /// <summary>
        /// ポインターが移動した状態でイベントが発生したか
        /// </summary>
        /// <param name="_self"></param>
        /// <returns></returns>
        public static bool IsMovingPointer( this BaseEventData _self )
        {
            if( _self is PointerEventData )
            {
                PointerEventData pointerEventData = _self as PointerEventData;

                return pointerEventData.delta != Vector2.zero;
            }

            return false;
        }

        public static bool IsEnableMouseLeftButton( this BaseEventData _self )
        {
            if( _self is PointerEventData )
            {
                PointerEventData pointerEventData = _self as PointerEventData;
        ...
        /// <returns></returns>
        public static int GetBitCount( this byte _value )
        {
            int result = _value;

            result = ( result & 0x55 ) + ( ( result >> 1 ) & 0x55 );
