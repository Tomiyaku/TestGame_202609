...
#pragma warning restore CS0414
      ...
      { new Rect(0,  40, 2048, 2692), new Rect(  0, 40, 2732, 2008) }   // iPad Pro 12.9 3rd
        ...
        /// <param name="_safeArea">SafeAreaに対応済みのRect</param>
        private bool GetSafeArea( out Rect _safeArea )
        {
            _safeArea = Rect.zero;
            ...
            for( int i = 0; i < Resolutions.Length; i++ )
            {
                ...
                {
                    _safeArea = GetAdjustRect( SafeAreaResolutions[ i, 0 ], size.x, size.y );
                    return true;
                }
                else if( height == size.x && width == size.y )
                {
                    _safeArea = GetAdjustRect( SafeAreaResolutions[ i, 1 ], size.x, size.y );
                    return true;
                }
            }
            //SafeAreaがない
        ...
        /// <param name="_h">画面高</param>
        /// <param name="_safeArea">SafeAreaに対応済みのRect</param>
        private bool GetSafeArea( int _w, int _h, out Rect _safeArea )
        {
            _safeArea = Rect.zero;
