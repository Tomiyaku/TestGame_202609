...
namespace CodeIcf.Extensions
{
    ...
    public static class StringExtensions
    {
        ...
        /// </summary>
        /// <remarks>
        /// ダブルクォーテーションで囲まれた改行、カンマは分割しないように対応<para></para>
        ...
        /// </summary>
        /// <param name="_target">結合対象の文字列リスト</param>
        /// <param name="_separeta">結合時に間に加える文字列</param>
        private static void CombineStringInDoubleQuotes( ref List<string> _target, string _separeta )
        {
            //ダブルクォーテーション中フラグ
            bool isDqFlag = false;
            ...
            {
                int dpIndex = _target[ i ].IndexOf( SINGLE_DOUBLE_QUOTATION );

                if( isDqFlag )
                {//ダブルクォーテーション中であれば１つ前の行と結合し削除
                    _target[ i - 1 ] += _separeta + _target[ i ];
                    _target.RemoveAt( i );
                    i--;

                    if( _target[ i ].EndsWith( SINGLE_DOUBLE_QUOTATION ) )
                    {//末尾にダブルクォーテーションがあり、ダブルクォーテーションが単独か3つ連続の場合はフラグを下す
                        if( _target[ i ].EndsWith( TRIPLE_DOUBLE_QUOTATION ) ) isDqFlag = false;
                        else if( !_target[ i ].EndsWith( DOUBLE_DOUBLE_QUOTATION ) ) isDqFlag = false;
                    }
                }
                else
                {
                    ...
                    while( dpIndex < _target[ i ].Length )
                    {
                        dpIndex = _target[ i ].IndexOf( SINGLE_DOUBLE_QUOTATION, dpIndex + 1 );
                        ...
                        {
                            if( dpIndex < _target[ i ].Length - 1 && _target[ i ][ dpIndex + 1 ] == SINGLE_DOUBLE_QUOTATION )
                            {//２重のダブルクォーテーションはスキップ
                                dpIndex++;
                            }
                            else
