    ...
    public class AssetBundleResourcesManager : MonoBehaviour
    {
        ...
        public static AssetBundleResourcesManager Instance
        {
            get
            {
                if( m_Instance == null )
                {
                    m_Instance = FindObjectOfType<AssetBundleResourcesManager>();
...
#pragma warning restore CS0162 // 到達できないコードが検出されました
        ...
        public AssetLoadResultCode LoadAssetBundle( AssetBundleCategory _category, out AssetBundleStorage _storage )
        {
            AssetBundleInfo defineInfo = GetAssetBundleDefineInfo( _category );
            _storage = GetAssetBundleStorage( _category );

            if( _storage == null )
            {
                _storage = new AssetBundleStorage( _category );
                m_AssetBundleStorageList.Add( _storage );
            }
            ...
            if( _storage.State == AssetBundleStorage.AssetBundleState.Enable ) return AssetLoadResultCode.Complited;
            if( _storage.State == AssetBundleStorage.AssetBundleState.Loading ) return AssetLoadResultCode.NowLoading;

            _storage.StartLoading();
            ...
            {//追加コンテンツ用AssetBundle
                ...
                //追加コンテンツ用の読み込み処理へ                    
                if( _storage.State != AssetBundleStorage.AssetBundleState.Enable ) StartCoroutine( LoadAocAssetBundleAsync( _category ) );
#else //UNITY_ONLY_SWITCH
                //通常の読み込み処理へ
                if( _storage.State != AssetBundleStorage.AssetBundleState.Enable ) StartCoroutine( LoadAssetBundleFromFileAsync( _category ) );
...
#pragma warning restore CS0162 // 到達できないコードが検出されました
        ...
        public AssetLoadResultCode LoadAsset( uint _identifier, out AssetStorage _storage )
        {
            AssetIdentifier.DivideIdentifier( _identifier, out AssetBundleCategory category, out uint index );

            return LoadAsset( category, index, out _storage );
        }
        ...
        /// <param name="_index">カテゴリ毎のAssetのインデックス</param>
        /// <returns>Assetとその状態を格納した<see cref="AssetStorage"/></returns>
        public AssetLoadResultCode LoadAsset( AssetBundleCategory _category, uint _index, out AssetStorage _storage )
        {
            _storage = null;
#if EDITOR_LOAD_ASSET_PATH
            Object asset = LoadAssetAtPath<Object>( GetAssetPath( _category, _index ) );
            _storage = new AssetStorage( _category, _index, asset );
#else
            _storage = GetAssetStorage( _category, _index );

            if( _storage.Asset != null ) _storage.Reuse();
            else StartCoroutine( LoadAssetAsync( _category, _index ) );
#endif
            if( _storage.State == AssetStorage.AssetState.Enable ) return AssetLoadResultCode.Complited;
            else if( _storage.State == AssetStorage.AssetState.Loading ) return AssetLoadResultCode.NowLoading;
