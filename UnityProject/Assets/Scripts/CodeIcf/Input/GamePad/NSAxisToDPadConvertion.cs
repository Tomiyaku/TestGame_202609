...
namespace CodeIcf.Input.GamePad
{
    ...
    /// </summary>
    public class NSAxisToDPadConvertion
    {
        ...
        /// <param name="_h">アナログスティック水平方向入力値</param>
        /// <param name="_v">アナログスティック垂直方向入力値</param>
        public void Update( float _h, float _v )
        {
            DPad result = DPad.None;

            if( _h > m_HorizonatalThreshold ) result |= DPad.Right;
            else if( _h < -m_HorizonatalThreshold ) result |= DPad.Left;

            if( _v > m_VarticalThreshold ) result |= DPad.Up;
            else if( _v < -m_VarticalThreshold ) result |= DPad.Down;
        ...
        /// 押下判定
        /// </summary>
        /// <param name="_key">方向キーの対応ビット 論理OR演算子で斜め判定も可能</param>
        /// <returns>押下直後ならtrue</returns>
        public bool IsDown( DPad _key )
        {
            return ( m_DownResult & _key ) == _key && ( m_PrevDownResult & _key ) != _key;
        }
        ...
        public bool IsHold( DPad _key )
        {
            return ( m_HoldResult & _key ) == _key;
        }
        ...
        /// <param name="_key">方向キーの対応ビット 論理OR演算子で斜め判定も可能</param>
        /// <returns></returns>
        public bool IsRepeat( DPad _key )
        {
            return ( m_RepeatResult & _key ) == _key;
        }
        ...
        /// <param name="_key">方向キーの対応ビット 論理OR演算子で斜め判定も可能</param>
        /// <returns></returns>
        public bool IsDownOrRepeat( DPad _key )
        {
            return IsDown( _key ) || IsRepeat( _key );
        }
